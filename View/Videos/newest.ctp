...
<div style="min-height:700px;padding:15px;">
	...
	<?php echo $this->element('channel', array('channel' => 'newest')); ?>
	<h3>Newest Posts</h3>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $('#show-more-button').click(function() {
            var request = $.ajax({
                url: '<?php echo $this->Html->url(array('controller' => 'videos', 'action' => 'newestAjax')); ?>',
                type: 'POST',
                dataType: 'json',
                data: {
                    'limit' : limit,
            ...
            request.done(function(response) {
                ...
                $.each(response, function(key, post) {
                    ...
                    html += '<div id="post_' + post.Post.id + '" class="small-4 left show-for-medium-only"><div class="summary">';
                    if (post.SiteChannel.id) {
                        html += '<div class="channel">' + post.SiteChannel.name + '</div>';
                    }
                    html += '<div class="duration">' + post.Post.duration.toHHMMSS() + '</div>';
